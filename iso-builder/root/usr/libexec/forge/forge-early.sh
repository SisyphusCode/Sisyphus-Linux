...
if command -v setfacl >/dev/null 2>&1; then
  ...
  for vt in /dev/tty0 /dev/tty1 /dev/tty2 /dev/tty3 /dev/console; do
    ...
    getent passwd cosmic-greeter >/dev/null 2>&1 \
      && setfacl -m "u:cosmic-greeter:rw,m::rw" "$vt" 2>/dev/null || true
    id "$desktop_user" >/dev/null 2>&1 \
      && setfacl -m "u:${desktop_user}:rw,m::rw" "$vt" 2>/dev/null || true
  done
  for dri in /dev/dri/card0 /dev/dri/card1 /dev/dri/renderD128 /dev/dri/renderD129; do
    [[ -e "$dri" ]] || continue
    getent group video >/dev/null 2>&1 && chgrp video "$dri" 2>/dev/null || true
    chmod g+rw "$dri" 2>/dev/null || true
    getent passwd cosmic-greeter >/dev/null 2>&1 \
      && setfacl -m "u:cosmic-greeter:rw" "$dri" 2>/dev/null || true
    id "$desktop_user" >/dev/null 2>&1 \
      && setfacl -m "u:${desktop_user}:rw" "$dri" 2>/dev/null || true
  done
fi
